...
// TODO - move all exports to the bottom of the file!
module.exports.noAppointmentMatchString =
    "no locations with available appointments";

module.exports.ScheduleUrl = function ScheduleUrl(siteName, zip) {
...
module.exports.CalendarUrl = function CalendarUrl(siteName) {
    return `https://${siteName.toLowerCase()}sched.rxtouch.com/rbssched/program/covid19/Calendar/PatientCalendar`;
};

module.exports.CalendarDayUrl = function CalendarDayUrl(siteName) {
...
module.exports.ScrapeRxTouch = async function ScrapeRxTouch(
    browser,
    site,
    siteName
) {
    ...
    for (const loc of [...new Set(site.locations)]) {
        if (!results[loc.zip]) {
            console.log(`Starting processing for ${loc.zip}...`);
            ...
            const calendarUrl = this.CalendarUrl(siteName);
            const calendarDayUrl = this.CalendarDayUrl(siteName);
